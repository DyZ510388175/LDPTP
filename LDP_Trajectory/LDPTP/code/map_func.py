...
def transition_index_2_mat_func(transition_index: tuple(tuple())):
    """
    Transition state index to grid index in Markov matrix
    """
    idx_1 = transition_index[0][0] * get_args().grid_num + transition_index[0][1]
    idx_2 = transition_index[1][0] * get_args().grid_num + transition_index[1][1]
    return (idx_1, idx_2)
...
def grid_index_inv_func(index: int, start2end_list: list, grid_map: GridMap):
    """
    Inverse function of grid_index_map_func
    """
    # start2end_dict 31: [((1, 3), (1, 3))
    # start2end_list  [((1, 3), (1, 3))
    start = start2end_list[index][0]
    end = start2end_list[index][1]

    return grid_map.map[start[0]][start[1]], grid_map.map[end[0]][end[1]]


def trajectory_grid_index_inv_func(trajectory: list, grid_map: GridMap):
    """
    Inverse function of grid_index_map_func
    """
    # print("grid_map:", grid_map)
    # print("grid_map[0][0]:", grid_map[0][0])
    map = grid_map.map
    return [map[grid_index[0]][grid_index[1]] for grid_index in trajectory]
