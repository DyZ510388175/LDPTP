...
if args.re_syn:
    ...
    """-----------3. 获取噪声过渡状态-------------"""
    em_server = ldp.EMServer(args.epsilon, grr_server.get_candidate_region())
    em_client = ldp.EMClient(args.epsilon, grr_server.get_candidate_region(), time_parts)
    ...
    for t in grid_trajectories:
        ...
        for i in range(len(t) - 1):
            ...
            "-----------3.1 获取过渡状态涉及的区域（索引）-------------"
            candidate_region_index = em_client.find_candidate_region_index(cur_point_gridAndtime,
            ...
            candidate_transitions = em_client.generate_candidate_transition(candidate_region_index)
            # print("3.2 candidate_transitions:", candidate_transitions)

            "-----------3.3 利用EM输出噪声过渡状态-------------"
            perturbed_transition = em_client.perturb(candidate_transitions, cur_point_gridAndtime,
                                                     next_point_gridAndtime)
    ...
    start2end_dict, start2end_pro = server_start2end_probability(em_server.get_corrected_start2end(),
                                                                 grid_trajectories_length)

    logger.info('Synthesizing...')
    synthetic_database = generate_synthetic_database(grid_trajectories_length,
                                                     one_level_mat, start2end_dict, start2end_pro,
                                                     len(db))
...
queries = [SquareQuery(grid_map.min_x, grid_map.min_y, grid_map.max_x, grid_map.max_y, size_factor=args.size_factor) for
