...
def trajectory_point2grid(t: List[Tuple[float, float, str]], g: GridMap, interp=True):
    """
    ...
    for p in range(len(t)):
        pointAndtime = t[p]
        # print("pointAndtime:", pointAndtime)
        found = False
        # Find which grid the point belongs to
        for i in range(len(grid_map)):
            for j in range(len(grid_map[i])):
                if grid_map[i][j].in_cell(pointAndtime):
                    grid_t.append((grid_map[i][j], pointAndtime[2]))
                    found = True
                    break
            if found:
                break
    ...
    for i in range(1, len(grid_t)):
        if not grid_t[i][0].index == grid_t_new[-1][0].index:
    ...
    if interp:
        ...
        for i in range(len(grid_t_new) - 1):
            ...
            if grid.is_adjacent_grids(current_grid[0], next_grid[0]):
...
def trajectory_grid2points(g_t: List[Grid]):
    if isinstance(g_t[0], tuple):
        if len(g_t) == 1:
            return [g_t[0][0].sample_point() for _ in range(2)]
        return [g_and_time[0].sample_point() for g_and_time in g_t]
    else:
        if len(g_t) == 1:
